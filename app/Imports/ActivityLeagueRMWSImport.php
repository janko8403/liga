...
class ActivityLeagueRMWSImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueRMWSData::query()->delete();
        foreach ($rows as $row) {
            $rowsTotal++;
            try {
                if ($row["nepu"]) {
                    $activityRow = ActivityLeagueRMWSData::firstOrCreate(['nepu' => $row['nepu'], 'activity_id' => $this->activity_id, 'edition_id' => $this->edition_id]);
