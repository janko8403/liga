...
class ActivityLeagueMWSImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueMWSData::query()->delete();
        foreach ($rows as $row) {
            try {
                if ($row["nepu"]) {
                    $activityRow = ActivityLeagueMWSData::firstOrCreate(['nepu' => $row['nepu'], 'activity_id' => $this->activity_id, 'edition_id' => $this->edition_id]);
