...
class ActivityLeagueMWPImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueMWPData::query()->delete();
        foreach ($rows as $row) {
            try {
                if ($row["nepu"]) {
                    $activityRow = ActivityLeagueMWPData::firstOrCreate(['nepu' => $row['nepu'], 'activity_id' => $this->activity_id, 'edition_id' => $this->edition_id]);
