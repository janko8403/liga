...
class ActivityLeagueDWSImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueDWSData::query()->delete();
        foreach ($rows as $row) {
            try {
                if ($row['nepu']!="") {
                    $activityRow = new ActivityLeagueDWSData();
                    ...
                    $activityRow->njs_dws = $row['njs_dws'];
                    $activityRow->dws = $row['dws'];
