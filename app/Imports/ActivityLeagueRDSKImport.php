...
class ActivityLeagueRDSKImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueRDSKData::query()->delete();
        foreach ($rows as $row) {
            try {
                if ($row["nepu"]) {
                    $activityRow = ActivityLeagueRDSKData::firstOrCreate(['nepu' => $row['nepu'], 'activity_id' => $this->activity_id, 'edition_id' => $this->edition_id]);
