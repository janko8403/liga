...
class ActivityLeagueMSKImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    ...
    public function collection(Collection $rows)
    {
        ...
        ActivityLeagueMSKData::query()->delete();

        foreach ($rows as $row) {
            try {
                if ($row["nepu"]) {
                    $activityRow = ActivityLeagueMSKData::firstOrCreate(['nepu' => $row['nepu'], 'activity_id' => $this->activity_id, 'edition_id' => $this->edition_id]);
                    ...
                    $activityRow->njs_msk = $row['njs_msk'];
                    $activityRow->msk = $row['msk'];
